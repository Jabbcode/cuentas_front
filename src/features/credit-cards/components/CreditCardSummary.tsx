...
export function CreditCardSummary({ statements }: CreditCardSummaryProps) {
  ...
  const currentPeriodTotal = statements.reduce((sum, s) => sum + s.currentPeriod.balance, 0);
    ...
    <div className="grid gap-4 lg:gap-6 lg:grid-cols-3">
      ...
      <Card>
        ...
        <CardContent className="p-4 pt-0 lg:p-6 lg:pt-0">
          ...
          <p className="text-xs text-gray-500 mt-1">{pendingCards} tarjeta(s) pendiente(s)</p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between p-4 pb-2 lg:p-6 lg:pb-2">
          <CardTitle className="text-sm font-medium text-gray-500">Total Usado</CardTitle>
          <div className="rounded-full bg-purple-100 p-2">
            <TrendingUp className="h-4 w-4 text-purple-600" />
          </div>
        </CardHeader>
        <CardContent className="p-4 pt-0 lg:p-6 lg:pt-0">
          <div className="text-2xl font-bold text-purple-600">{formatCurrency(totalUsed)}</div>
          <p className="text-xs text-gray-500 mt-1">Entre {statements.length} tarjeta(s)</p>
        </CardContent>
      </Card>
    </div>
  );
