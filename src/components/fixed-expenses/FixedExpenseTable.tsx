...
export function FixedExpenseTable({
  ...
  const getStatusBadge = (item: FixedExpenseWithStatus) => {
    ...
    if (item.isPaidThisMonth) {
      return <Badge variant="success"><Check className="mr-1 h-3 w-3" /> Pagado</Badge>;
    }
    if (overdue) {
  ...
  const renderMobileCard = (item: FixedExpenseWithStatus) => {
    const overdue = isOverdue(item.dueDay) && !item.isPaidThisMonth;
  ...
  const renderDesktopRow = (item: FixedExpenseWithStatus) => {
    const overdue = isOverdue(item.dueDay) && !item.isPaidThisMonth;
    const dueSoon = isDueSoon(item.dueDay) && !item.isPaidThisMonth && !overdue;
    ...
    return (
      ...
      >
        ...
        <td className="px-3 py-2 text-center">
          <span className="font-medium text-gray-700">{item.dueDay}</span>
        </td>
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            <div className="relative">
              ...
              {menuOpen === item.id && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setMenuOpen(null)} />
                  <div className="absolute right-0 top-7 z-20 w-32 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
                    <button
                      className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs text-gray-700 hover:bg-gray-100"
                      onClick={() => { setMenuOpen(null); onEdit(item.id); }}
                    >
    ...
    <>
      ...
      {payingItem && (
        <Dialog open onClose={() => setPayingItem(null)}>
          <DialogHeader>
            <DialogTitle>Registrar Pago: {payingItem.name}</DialogTitle>
          </DialogHeader>
          <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirmPay}>Confirmar Pago</Button>
          </DialogFooter>
        </Dialog>
      )}
    </>
