...
export function CreditCardsSummaryCard({ summary }: CreditCardsSummaryProps) {
  ...
  const getSeverityIcon = (severity: 'info' | 'warning' | 'error') => {
    switch (severity) {
      case 'error':
        return <AlertCircle className="h-4 w-4 text-red-600 dark:text-red-600" />;
      case 'warning':
        return <AlertTriangle className="h-4 w-4 text-orange-600" />;
      case 'info':
        return <Info className="h-4 w-4 text-blue-600 dark:text-blue-600" />;
    }
  };
  ...
  const getSeverityColor = (severity: 'info' | 'warning' | 'error') => {
    switch (severity) {
      case 'error':
        return 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-600/10 text-red-800 dark:text-red-600';
      case 'warning':
        return 'border-orange-200 bg-orange-50 text-orange-800';
      case 'info':
        return 'border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-600/10 text-blue-800 dark:text-blue-600';
    }
  };
      ...
      <CardHeader className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between lg:p-6">
        <div className="flex items-center gap-3">
          <div className="rounded-full bg-purple-100 dark:bg-purple-600/10 p-2">
            <CreditCard className="h-4 w-4 text-purple-600 dark:text-purple-600 lg:h-5 lg:w-5" />
          </div>
          <div>
            <CardTitle className="text-base lg:text-lg">Tarjetas de Crédito</CardTitle>
            <p className="text-xs text-gray-500 dark:text-gray-500 lg:text-sm">
      ...
      <CardContent className="p-4 pt-0 lg:p-6 lg:pt-0 space-y-4">
        ...
        {summary.totalToPay > 0 && (
          <div className="rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-500/10 dark:to-pink-500/10 p-4">
            <p className="text-sm font-medium text-gray-600 dark:text-gray-500">Total a pagar este mes</p>
            <p className="mt-1 text-2xl font-bold text-purple-600 dark:text-purple-600 lg:text-3xl">
        ...
        {summary.upcomingPayments.length > 0 && (
          <div>
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-500 mb-2">Próximos vencimientos</h4>
                ...
                <div
                  ...
                  className={`flex items-center justify-between rounded-lg border p-3 ${
                    payment.daysUntilDue <= 3
                      ...
                      ? 'border-orange-200 dark:border-yellow-800 bg-orange-50 dark:bg-yellow-600/10'
                      : 'border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700'
                  }`}
                >
                  <div>
                    <p className="font-medium text-gray-900 dark:text-gray-900">{payment.accountName}</p>
        ...
        {summary.alerts.length > 0 && (
          <div>
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-500 mb-2">Alertas</h4>
            <div className="space-y-2">
        ...
        {summary.totalToPay === 0 && summary.upcomingPayments.length === 0 && (
          <div className="rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-600/10 p-4 text-center">
