...
export function DebtsSummaryCard({ summary }: DebtsSummaryCardProps) {
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-lg font-semibold text-green-900 dark:text-green-50">
                Sin Deudas Activas
              </CardTitle>
            </div>
          </div>
  ...
  return (
    <Card className={cn('transition-all', hasOverdue && 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/30')}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn(
              'rounded-full p-2',
              hasOverdue ? 'bg-red-100 dark:bg-red-900/30' : 'bg-orange-100 dark:bg-orange-900/30'
            )}>
              <Receipt className={cn(
                'h-5 w-5',
                hasOverdue ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400'
              )} />
            </div>
            <div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-300">Total Adeudado</p>
        ...
        <div className="space-y-2">
          {/* Overdue Alert */}
          {hasOverdue && (
            <div className="flex items-start gap-2 rounded-lg bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-3">
          ...
          {!hasOverdue && !hasDueSoon && summary.totalActiveDebts > 0 && (
            <div className="flex items-start gap-2 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 p-3">
        ...
        {summary.upcomingDebts.length > 0 && (
          <div className="space-y-2">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Próximas a vencer:</p>
            {summary.upcomingDebts.slice(0, 3).map((debt) => (
              <div
                key={debt.id}
                className="flex items-center justify-between rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-2.5"
              >
